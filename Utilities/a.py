...
def main(path, criteria=CRITERIA, n_structs=500, sort_order="max", out_freq=FREQ):
    """
    ...
    """
    # Get Files
    all_reports_noflatten = [ glob.glob(os.path.join(folder, "*/*report*")) for folder in path ]
    all_reports = [item for sublist in all_reports_noflatten for item in sublist]
    print(all_reports)
    ...
    try:
		...
		raise KeyError("Adaptive Exit didn't finish.")
    print(max_sasa_info)
...
def parse_values(reports, n_structs, criteria, sort_order):
    ...
    for file in reports:
        ...
        report_values = report_values[report_values[criteria]]
        try:
